...
func TestPoolBuildFilesystemOverwriteDataWrongUuid(test *testing.T) {
	helper := helperTest{
		test: test,
	}
	...
	poolConnection, errorHttpRequestPrecondition, isErrorHttpRequestPrecondition := helperTestCreateRestApiConnection(
		...
		"/storage-pool/build",
		http.MethodPatch,
		fmt.Appendf(nil, "{\"uuid\":\"%s\",\"option\":%d}", "ca3e11bd-b840-4412-9f44-89b8d09b9f4e", libvirt.STORAGE_POOL_BUILD_OVERWRITE),
		&incomingRequest,
	)
	if isErrorHttpRequestPrecondition {
		test.Fatalf("http request precondition failed: %s", errorHttpRequestPrecondition.Message)
	...
	errorPoolBuild, isErrorPoolBuild := poolConnection.PoolBuild(incomingRequest.Uuid, incomingRequest.Option)
	if isErrorPoolBuild {
		test.Logf("build and overwrite existing filesystem pool test failed: %s", errorPoolBuild.Message)
	}
	if !isErrorPoolBuild {
		test.Error("this test must be failed, but it won't")
	}
...
func TestPoolBuildFilesystemOverwriteDataUuidNotValid(test *testing.T) {
	helper := helperTest{
		test: test,
	}
	...
	poolConnection, errorHttpRequestPrecondition, isErrorHttpRequestPrecondition := helperTestCreateRestApiConnection(
		...
		"/storage-pool/build",
		http.MethodPatch,
		fmt.Appendf(nil, "{\"uuid\":\"%s\",\"option\":%d}", "ca3e11bd-b840-4412-9f44-", libvirt.STORAGE_POOL_BUILD_OVERWRITE),
		&incomingRequest,
	)
	if isErrorHttpRequestPrecondition {
		test.Fatalf("http request precondition failed: %s", errorHttpRequestPrecondition.Message)
